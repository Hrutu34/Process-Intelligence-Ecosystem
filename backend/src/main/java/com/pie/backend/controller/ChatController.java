...
public class ChatController {
    ...
    /**
     * If only BPMN XML is present in the incoming context, enrich the record with
     * parsed graph + knowledge + quality report so the LLM never has to reason from raw XML.
     */
    private ChatContext enrichContext(ChatContext ctx) {
        ...
        try {
            ...
            if (needsReport && graph != null) {
                try { report = qualityValidator.validateQuality(graph); }
                catch (Exception ignored) {}
            }
            return new ChatContext(name, ctx.bpmnXml(), knowledge, graph, report);
        } catch (Exception e) {
